...
export const FacultyPapers = ({ onOpenProfile, onAddPaper }) => {
  ...
  const filteredPapers = researchPapers.filter(p => {
    const hasFaculty = Array.isArray(p.facultyAuthors) && p.facultyAuthors.length > 0;
    const isFacultyTargeted = p.targetAudience === 'faculty' || p.targetAudience === 'all';

    if (!hasFaculty && (p.targetAudience === 'students' || p.targetAudience === 'student')) return false;

    const isFacultyPaper = hasFaculty || isFacultyTargeted || (!p.targetAudience && (!p.studentAuthors || p.studentAuthors.length === 0));
  ...
  if (selectedPaper) {
    const facAuthors = (selectedPaper.facultyAuthors || []).map(fid => {
      ...
      const fallbackName = cleanNameCandidate || (typeof idOrName === 'string' && !idOrName.startsWith('FAC-') ? idOrName : (typeof fid === 'object' && fid?.name ? fid.name : (typeof idOrName === 'string' && idOrName.startsWith('FAC-') ? `Faculty (${idOrName})` : "Faculty Author")));
      const dept = (typeof fid === 'object' && fid?.department) ? fid.department : "Physics & Quantum Computing";
      const title = (typeof fid === 'object' && fid?.title) ? fid.title : "Faculty Author";
    ...
    const stuAuthors = (selectedPaper.studentAuthors || []).map(sid => {
      ...
      const fallbackName = cleanNameCandidate || (typeof idOrName === 'string' && !idOrName.startsWith('STU-') && !idOrName.startsWith('QU-') ? idOrName : (typeof sid === 'object' && sid?.name ? sid.name : (typeof idOrName === 'string' && idOrName.startsWith('STU-') ? `Student (${idOrName})` : "Student Co-Author")));
